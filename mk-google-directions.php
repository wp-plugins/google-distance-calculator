...
function mkgd_footer() {
  ...
  <?php
  // If start and destination are set
  if ((get_option('mkgd_show_start_point') == 1) && (get_option('mkgd_show_start_point') == 1)) {
    ?>
    <script type="text/javascript">
      jQuery("document").ready(function() {
        calcRoute();
        jQuery("#btnMkgdSubmit").trigger('click');
      });
    </script>
    <?php
  }
...
function mkgd_initialize() {
  ...
  // Hide/Show destination location
  if (get_option('mkgd_show_destination_point') == 1) {
    $output .= '<input id = "destination" name = "destination" value = "' . get_option('mkgd_default_destination_point') . '" type = "hidden"/>';
  } else {
    $output .= '<li><label for="destination">' . __("Destination") . '</label><input id="destination" name="destination" type="text" size="50" value = "' . get_option('mkgd_default_destination_point') . '" /></li>';
  }
  // Show/Hide Button
  if ((get_option('mkgd_show_start_point') == 1) && (get_option('mkgd_show_start_point') == 1)) {
    $output .= '<input type="hidden" name="btnMkgdSubmit" id="btnMkgdSubmit" value="' . __("Get Directions") . '"/>';
  } else {
    $output .= '<li><input type="button" onclick="calcRoute();" name="btnMkgdSubmit" id="btnMkgdSubmit" value="' . __("Get Directions") . '"/></li>';
  }
  $output .= '</ul><!-- End .mkgd-form -->
